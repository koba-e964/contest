...
// Author: hitonanode
// Ref: https://judge.yosupo.jp/submission/57200
// y |-> a where y(i) = \sum_{j <= i} a[j] i!/(i-j)!
fn sample_to_factorial(
    a: &[MInt],
    gen: MInt, _fac: &[MInt], invfac: &[MInt],
) -> Vec<MInt> {
    let n = a.len();
    let mut p = 1;
    while p < n {
    ...
    let mut f = vec![MInt::new(0); 2 * p];
    let mut g = vec![MInt::new(0); 2 * p];
    for i in 0..n {
        f[i] = a[i] * invfac[i];
        g[i] = if i % 2 == 0 { invfac[i] } else { -invfac[i] };
    }
...
// Author: hitonanode
// Ref: https://judge.yosupo.jp/submission/57200
// a |-> y where y(i) = \sum_{j <= i} a[j] i!/(i-j)!
fn factorial_to_sample(
    a: &[MInt],
    gen: MInt, fac: &[MInt], invfac: &[MInt],
) -> Vec<MInt> {
    let n = a.len();
    let mut p = 1;
    while p < n {
...
// https://github.com/yosupo06/library-checker-problems/issues/680#issuecomment-879544125 
fn main() {
    ...
    let mut f = sample_to_factorial(&f, 3.into(), &fac, &invfac);
    for i in 0..n {
        f[i] *= fac[i];
    }
    f.resize(p, 0.into());
    ...
    f.resize(m, 0.into());
    let vals = factorial_to_sample(&f, 3.into(), &fac, &invfac);
    putvec!(vals);
}
