...
fn bernoulli_naive(n: usize, fac: &[MInt], invfac: &[MInt]) -> Vec<MInt> {
    let mut dp = vec![MInt::new(0); n];
    dp[0] = 1.into();
    for i in 1..n {
        let mut tmp = MInt::new(0);
        for j in 0..i {
            tmp -= fac[i + 1] * invfac[i + 1 - j] * invfac[j] * dp[j];
        }
        dp[i] = tmp * invfac[i + 1] * fac[i];
    }
    dp[1] += 1;
    dp
}

// Tags: bernoulli-number, inclusion-exclusion-on-divisors, dirichlet-convolution, multiplicative-function
...
fn main() {
    ...
    input! {
        t: usize,
        nxy: [(i64, usize, usize); t],
    }
    const W: usize = 3010;
    let (fac, invfac) = fact_init(W + 1);
    let ber = bernoulli_naive(W, &fac, &invfac);
    use std::time::*;
    for (n, x, y) in nxy {
        let mut now = Instant::now();
        let pe = pollard_rho::factorize(n);
        eprintln!("divs: {:?}", now.elapsed());
        let f = |i: usize, p: i64, u: usize| -> MInt {
            let val = MInt::new(p).pow(i as i64 * u as i64);
            let mut sum = val;
            let step = MInt::new(p).pow(MOD - 1 + x as i64 - i as i64);
            for _ in 0..u {
                sum = sum * step + val;
            }
            sum
        ...
        now = Instant::now();
        // inclusion-exclusion-principle on divisors
        let mut ans = MInt::new(0);
        for i in 1..y + 2 {
            let mut tmp = MInt::new(1);
            for &(p, e) in &pe {
                let a = f(i, p, e) - f(i, p, e - 1) * MInt::new(p).pow(y as i64);
                tmp *= a;
            }
            if i == 0 {
                ans += tmp;
            } else {
                ans += ber[y + 1 - i] * fac[y] * invfac[i] * invfac[y + 1 - i]
                    * tmp;
            }
        }
        puts!("{}\n", ans * MInt::new(n).pow(y as i64));
        eprintln!("output: {:?}", now.elapsed());
    }
}
