...
// Fubini numbers = \sum_{0 <= k <= n} surj(n, k)
// http://oeis.org/A000670
// e.g.f = 1/(2 - e^x)
// n must be a power of two
fn fubini(n: usize, gen: MInt, fac: &[MInt], invfac: &[MInt]) -> Vec<MInt> {
    let mut a = vec![MInt::new(0); n];
    a[0] = 1.into();
    for i in 1..n {
        a[i] = -invfac[i];
    }
    let mut ans = fps_inv(&a, gen);
    for i in 0..n {
        ans[i] *= fac[i];
    }
    ans
...
// \sum_{1 <= k <= n} k * surj(n, k)
// http://oeis.org/A069321
// e.g.f = (e^x - 1)/(2 - e^x)^2
// n must be a power of two
fn ksurj(n: usize, gen: MInt, fac: &[MInt], invfac: &[MInt]) -> Vec<MInt> {
    let mut a = vec![MInt::new(0); n];
    a[0] = 1.into();
    for i in 1..n {
        a[i] = invfac[i] * (MInt::new(2).pow(i as i64) - 4);
    }
    let mut inv = fps_inv(&a, gen);
    inv.resize(2 * n, 0.into());
    let mut num = vec![MInt::new(0); 2 * n];
    for i in 1..n {
        num[i] = invfac[i];
    }
    let zeta = gen.pow((MOD - 1) / 2 / n as i64);
...
fn main() {
    ...
    let fubini = fubini(W, 3.into(), &fac, &invfac);
    let ksurj = ksurj(W, 3.into(), &fac, &invfac);
    eprintln!("{:?} {:?}", &fubini[..10], &ksurj[..10]);
    for n in n {
        puts!("{}\n", ksurj[n] * fubini[n].inv());
    }
}
