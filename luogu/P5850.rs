...
// Finds \sum_{1 <= i <= k} i^n for all 0 <= n < m
fn powsum(m: usize, k: MInt, gen: MInt, fac: &[MInt], invfac: &[MInt]) -> Vec<MInt> {
    let mut p = 1;
    while p < m {
        p *= 2;
    }
    let mut ber = bernoulli(p, gen, fac, invfac);
    for i in 0..p {
        ber[i] *= invfac[i];
    }
    ber.resize(2 * p, 0.into());
...
// Finds \prod_{j=1}^k (1+jx) mod x^n
fn linear_prod(n: usize, k: MInt, gen: MInt, fac: &[MInt], invfac: &[MInt]) -> Vec<MInt> {
    let mut p = 1;
    while p < n {
        p *= 2;
    }
    let psum = powsum(n, k, gen, fac, invfac);
    let mut ln = vec![MInt::new(0); p];
    for i in 1..n {
        ln[i] = psum[i] * invfac[i] * fac[i - 1];
        if i % 2 == 0 {
            ln[i] = -ln[i];
        }
    }
    let mut ans = fps_exp(&ln, gen, fac, invfac);
    ans.truncate(n);
    ans
}
...
fn main() {
    ...
    let ans = linear_prod(m + 1, k.into(), 3.into(), &fac, &invfac);
    for n in 1..m + 1 {
        puts!("{}\n", ans[n] * fac[n]);
    }
}
