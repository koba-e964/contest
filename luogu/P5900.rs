...
// Ref: http://oeis.org/A000081
fn unlabeled_rooted_trees(
    ...
    a[1] = 1.into();
    while c <= n {
        // bx = x * exp(A(x^2)/2 + A(x^3)/3 + ...)
        // bx * exp(A(x)) = x * exp(A(x) + A(x^2)/2 + A(x^3)/3 + ...)
        let mut bxln = vec![MInt::new(0); 2 * c];
        for i in 1..2 * c {
            for j in 1..min(c, (2 * c - 1) / i + 1) {
                bxln[i * j] += a[j] * fac[i - 1] * invfac[i];
            }
        }
        let mut bx = formal_power_series_exp(&bxln, gen, &fac, &invfac);
        bx.insert(0, 0.into());
        bx.pop();
        ...
        let mut a1 = formal_power_series_inv(&a1, gen);
...
// Tags: newtons-method, counting-unlabeled-rooted-trees, counting-unlabeled-trees, generating-functions, multiset-counting
fn main() {
    let n: usize = get();
    let (fac, invfac) = fact_init(2 * n);
    let gen = MInt::new(3);
    let urt = unlabeled_rooted_trees(n, gen, &fac, &invfac);
    let mut tot = MInt::new(0);
    if n % 2 == 0 {
        // there are two centroids
        let tmp = urt[n / 2];
        tot += tmp * (tmp + 1) * MInt::new(2).inv();
    }
    // there is one centroid
    let c = urt.len();
    let mut dp = vec![MInt::new(0); c];
    for j in 1..c {
        for i in 1..min((c - 1) / j, (n - 1) / 2) + 1 {
            dp[j * i] += urt[i] * fac[j - 1] * invfac[j];
        }
    }
    let dp = formal_power_series_exp(&dp, gen, &fac, &invfac);
    tot += dp[n - 1];
    println!("{}", tot);
}
