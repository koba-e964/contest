...
fn labeled_trees_with_deg_max(
    ...
    let mut g = fps_inv(&g, gen);
    g.insert(0, 0.into());
    let mut f = vec![MInt::new(0); p];
    for i in 0..m + 1 {
        f[i] = invfac[i];
    }
    fps_lagrange(n - 1, &f, &g, gen, fac, invfac) * fac[n - 1]
}

// Tags: lagrange-inversion, counting-labeled-trees
fn main() {
    input!(n: usize, m: usize);
    let (fac, invfac) = fact_init(2 * max(n, m) + 1);
    println!("{}", labeled_trees_with_deg_max(n, m, 3.into(), &fac, &invfac)
             - labeled_trees_with_deg_max(n, m - 1, 3.into(), &fac, &invfac));
}
