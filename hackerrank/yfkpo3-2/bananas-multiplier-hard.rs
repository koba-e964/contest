...
fn dfs(v: usize, par: usize, gc: &[Vec<(usize, i64)>], dp: &mut [ModInt],
       e: ModInt) {
    dp[v] = e;
    for &(w, c) in &gc[v] {
        if w == par { continue; }
...
fn solve() {
    ...
    input! {
        n: usize,
        uvc: [(usize1, usize1, i64); n - 1],
        q: usize,
        mpx: [(usize1, usize1, i64); q],
    }
    let mut g = vec![vec![]; n];
    ...
    dfs(0, n, &gc, &mut dp, ModInt::new(1));
    let mut qs = vec![(0, 0); q];
    for i in 0..q {
        let (m, p, _x) = mpx[i];
        qs[i] = (m, p);
    }
    let ans = offline_lca(&g, &[0], &qs);
    for i in 0..q {
        let (m, p, x) = mpx[i];
        let lca = ans[i];
        puts!("{}\n", dp[m] * dp[p] * dp[lca].inv().pow(2) * x);
    }
}
