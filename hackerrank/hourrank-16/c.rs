...
fn dfs(v: usize, par: usize, g: &[Vec<(usize, ModInt)>], dd: ModInt, de: ModInt) -> ModInt {
    ...
    for &(w, c) in g[v].iter() {
        if w == par { continue; }
        tot += dfs(w, v, g, dd + c, de + ModInt::new(1));
    }
    tot
}
...
fn solve() {
    ...
    for (u, v, w) in uvw {
        g[u].push((v, ModInt::new(w)));
        g[v].push((u, ModInt::new(w)));
    }
    let mut tot = ModInt::new(0);
    for i in 0 .. n {
        tot += dfs(i, n, &g, ModInt::new(0), ModInt::new(1));
    }
    for i in 1 .. n + 1 {
        tot *= ModInt::new(i as i64);
    }
    puts!("{}\n", tot);
}
