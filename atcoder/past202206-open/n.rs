...
fn solve() {
    ...
    for i in 0..h {
        for j in 0..w {
            let v = i * w + j;
            din.add_edge(2 * v, 2 * v + 1, c[i][j]);
            din.add_edge(2 * v + 1, 2 * v, c[i][j]);
            if i > 0 {
                din.add_edge(2 * v + 1, 2 * v - 2 * w, inf);
                din.add_edge(2 * v - 2 * w + 1, 2 * v, inf);
            }
            if j > 0 {
                din.add_edge(2 * v + 1, 2 * v - 2, inf);
                din.add_edge(2 * v - 2 + 1, 2 * v, inf);
            }
        }
    }
    let (ma, cut) = din.max_flow(1, 2 * h * w - 2);
    println!("{}", ma);
    for i in 0..h {
        let mut ans = "".to_string();
        for j in 0..w {
            let v = i * w + j;
            ans.push(if cut.is_cut(2 * v, 2 * v + 1) { '#' } else { '.' });
        }
        println!("{}", ans);
    }
}
