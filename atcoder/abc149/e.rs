...
fn solve() {
    ...
    const W: usize = 1 << 18;
    let mut freq1 = vec![ModInt1::new(0); W];
    let mut freq2 = vec![ModInt2::new(0); W];
    for &a in &a {
        freq1[a] += 1;
        freq2[a] += 1;
    }
    let zeta1 = ModInt1::new(3).pow((MOD1 - 1) / W as i64);
    let zeta2 = ModInt2::new(3).pow((MOD2 - 1) / W as i64);
    transform(&mut freq1, zeta1, 1.into());
    transform(&mut freq2, zeta2, 1.into());
    for i in 0..W {
        freq1[i] = freq1[i] * freq1[i];
        freq2[i] = freq2[i] * freq2[i];
    }
    transform(&mut freq1, zeta1.inv(), 1.into());
    transform(&mut freq2, zeta2.inv(), 1.into());
    {
        let fac1 = ModInt1::new(W as i64).inv();
        let fac2 = ModInt2::new(W as i64).inv();
        for i in 0..W {
            freq1[i] *= fac1;
            freq2[i] *= fac2;
        }
