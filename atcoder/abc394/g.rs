...
fn solve() {
    ...
    input! {
        ...
        f: [[usize; w]; h],
        q: usize,
        abycdz: [(usize1, usize1, usize, usize1, usize1, usize); q],
    }
    let mut pass = vec![0; q];
    const W: usize = 1 << 20;
    let mut fail = vec![W; q];
    let mut con = vec![vec![]; W];
    for i in 0..h - 1 {
        for j in 0..w {
            let idx = i * w + j;
            let t = f[i][j].min(f[i + 1][j]);
    ...
    for i in 0..q {
        let (_, _, y, _, _, z) = abycdz[i];
        let dist = if pass[i] >= y.max(z) {
            (y as i32 - z as i32).abs()
        } else {
            (y as i32 - pass[i] as i32).abs() + (z as i32 - pass[i] as i32).abs()
        };
        puts!("{dist}\n");
    }
}
