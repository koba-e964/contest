...
fn main() {
    ...
    input! {
        n: usize, b: i64, q: usize,
        a: [i64; n],
        cx: [(usize1, i64); q],
    }
    let mut st = SegTree::new(n, |(x, mx), (y, my)| (x + y, max(mx, x + my)), (0i64, -1i64 << 50));
    for i in 0..n {
        st.update(i, (a[i] - b, a[i] - b));
    }
    for (c, x) in cx {
        st.update(c, (x - b, x - b));
        let idx = st.max_right(0.., &|x| x.1 < 0);
        let idx = min(idx + 1, n);
        puts!("{}\n", st.query(0..idx).0 as f64 / idx as f64 + b as f64);
    }
}
