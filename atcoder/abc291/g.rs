...
fn main() {
    ...
    let mut f = vec![vec![MInt::new(0); n]; 32];
    let mut g = vec![vec![MInt::new(0); n]; 32];
    for i in 0..n {
        f[a[i]][i] += 1;
        g[b[i]][n - i - 1] += 1;
    }
    ...
    let mut conv = vec![];
    for i in 0..32 {
        let mut tmp = fps.mul(std::mem::replace(&mut f[i], vec![]), std::mem::replace(&mut g[i], vec![]));
        for j in n..tmp.len() {
            tmp[j - n] = tmp[j - n] + tmp[j];
        }
        tmp.truncate(n);
        conv.push(tmp);
    }
