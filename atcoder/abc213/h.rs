...
fn induce(l: usize, mid: usize, r: usize, abp: &[(usize, usize, Vec<i64>)],
          u: &mut [Vec<MInt>]) {
    let len = r - l;
    let zeta = MInt::new(3).pow((MOD - 1) / len as i64);
    let fac = MInt::new(len as i64).inv();
    for &(a, b, ref p) in abp {
        let mut k = vec![MInt::new(0); len];
        for i in 0..min(p.len(), len) {
            k[i] = p[i].into();
        }
        fft::fft(&mut k, zeta, 1.into());
        let mut c = vec![MInt::new(0); len];
        for i in l..mid {
            c[i - l] = u[a][i];
        }
        fft::fft(&mut c, zeta, 1.into());
        for i in 0..len {
            c[i] *= k[i] * fac;
        }
        fft::inv_fft(&mut c, zeta.inv(), 1.into());
        for i in mid..r {
            u[b][i] += c[i - l];
        }
        let mut c = vec![MInt::new(0); len];
...
fn dfs(l: usize, r: usize, abp: &[(usize, usize, Vec<i64>)],
       u: &mut [Vec<MInt>]) {
    if l + 1 == r {
        return;
    }
...
fn solve() {
    input! {
        ...
        abp: [(usize1, usize1, [i64; t]); m],
    }
    let mut abp = abp;
    for x in &mut abp {
        x.2.insert(0, 0);
    }
    let mut u = vec![vec![MInt::new(0); W]; n];
    u[0][0] = 1.into();
    dfs(0, W, &abp, &mut u);
    println!("{}", u[0][t]);
}
