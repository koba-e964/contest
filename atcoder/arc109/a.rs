...
fn solve() {
    ...
    input! {
        a: usize1, b: usize1, x: i64, y: i64,
    }
    const W: usize = 100;
    let mut dijk = Dijkstra::new(2 * W);
    for i in 0..W {
        dijk.add_edge(i, i + W, x);
        dijk.add_edge(i + W, i, x);
    }
    for i in 0..W - 1 {
        dijk.add_edge(i + 1, i + W, x);
        dijk.add_edge(i + W, i + 1, x);
        for j in 0..2 {
            let b = W * j;
            dijk.add_edge(i + b, i + 1 + b, y);
            dijk.add_edge(i + 1 + b, i + b, y);
        }
    }
    puts!("{}\n", dijk.solve(a, 1 << 50)[W + b]);
}
