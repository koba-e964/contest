...
fn solve() {
    ...
    let k = 1_000_000;
    let mut mcf = MinCostFlow::new(2 * (n + 1) * (n + 1));
    for i in 0..n + 1 {
        for j in 0..n + 1 {
            let v = i * (n + 1) + j;
            mcf.add_edge(2 * v, 2 * v + 1, 1, k - a[i][j]);
            mcf.add_edge(2 * v, 2 * v + 1, 1, k);
            if i > 0 {
                mcf.add_edge(2 * v - 2 * (n + 1) + 1, 2 * v, 2, 0);
            }
            if j > 0 {
                mcf.add_edge(2 * v - 1, 2 * v, 2, 0);
            }
        }
    }
    let a = mcf.min_cost_flow(0, 2 * (n + 1) * (n + 1) - 1, 2);
    println!("{}", 2 * (2 * n as i64 + 1) * k - a);
}
