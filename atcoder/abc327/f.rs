...
// Tags: 2d-sparse-max, lazy-segment-tree
fn main() {
    input! {
        n: usize, d: usize, w: usize,
        tx: [(usize, usize); n],
    }
    const B: usize = 200_100;
    let mut st = LazySegTree::<AddMax>::new(B);
    let mut ev = vec![vec![]; B];
    for (t, x) in tx {
        ev[t].push((x, 1));
        if t + d < B {
            ev[t + d].push((x, -1));
    ...
    for i in 0..B {
        ev[i].sort_unstable();
        for &(x, s) in &ev[i] {
            let lo = std::cmp::max(w - 1, x) - (w - 1);
            st.update(lo..x + 1, s);
        }
        ans = std::cmp::max(ans, st.query(0..B));
    }
    println!("{}", ans);
}
