...
fn solve() {
    ...
    const INF: i64 = 1 << 40;
    let mut din = Dinic::new(n * n + 2, 0i64);
    let mut cols = vec![vec![0; n]; n];
    for i in 0..n {
        for j in 0..n {
            if c[i][j] != '?' {
                let col = (c[i][j] == 'B') ^ ((i + j) % 2 != 0);
                cols[i][j] = if col { 1 } else { 0 };
            } else {
                cols[i][j] = 2;
            }
        }
    ...
    for i in 0..n {
        for j in 0..n {
            if cols[i][j] != 2 {
                if cols[i][j] == 0 {
                    din.add_edge(0, 2 + i * n + j, INF);
                } else {
                    din.add_edge(2 + i * n + j, 1, INF);
                }
            }
            let v = 2 + i * n + j;
            if j > 0 {
                din.add_edge(v, v - 1, 1);
                din.add_edge(v - 1, v, 1);
                ne += 1;
            }
            if i > 0 {
                din.add_edge(v, v - n, 1);
                din.add_edge(v - n, v, 1);
                ne += 1;
            }
        }
    }
    let (f, _) = din.max_flow(0, 1);
    puts!("{}\n", ne - f);
