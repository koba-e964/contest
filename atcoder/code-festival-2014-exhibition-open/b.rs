...
fn main() {
    ...
    input! {
        q: usize,
        s: chars,
        xyz: [(char, usize, usize); q],
    }
    let mut t = SeqAVLTree::<(i64, i64), _>::new(
        |(x, y), (z, w)| (min(x, y + z), y + w),
        (0, 0)
    );
    fn getval(c: char) -> (i64, i64) {
        if c == '(' { (0, 1) } else { (-1, -1) }
    }
    for i in 0..s.len() {
        t.insert(getval(s[i]), i);
    }
    for &(x, y, z) in &xyz {
        if x == '(' || x == ')' {
            t.insert(getval(x), y - 1);
            continue;
        }
        if x == 'D' {
            t.erase(y - 1);
