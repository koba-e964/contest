...
fn calc<F: Fn((usize, usize), (usize, usize)) -> (usize, usize)>
    (st: &SegTree<(usize, usize), F>, k: usize) -> usize
{
    let mut pass = W;
    let mut fail = 0;
    // TODO naive bisection
    while pass - fail > 1 {
        let mid = (pass + fail) / 2;
        if st.query(0, mid).1 >= k {
            pass = mid;
        } else {
            fail = mid;
        }
    }
    let (acc, count) = st.query(0, pass);
    acc - (count - k) * (pass - 1)
}
...
fn solve() {
    ...
    let mut ci: Vec<_> = c.iter().cloned().enumerate().map(|(i, c)| (c, i))
        .collect();
    ci.sort();
    let mut inv = vec![0; n];
    for i in 0..n {
    ...
    let mut cs = TwoThreeTree::new();
    for &c in &c {
        cs = cs.insert(c);
    }
    let mut st = SegTree::new(W, |(a, b), (c, d)| (a + c, b + d), (0, 0));
    for i in 0..n - 1 {
        let dif = ci[i + 1].0 - ci[i].0;
        let (a, b) = st.query(dif, dif + 1);
        st.update(dif, (a + dif, b + 1));
    }
    puts!("{}\n", calc(&st, n - k));
    let mut rem = n - k;
    for &d in &d {
        ...
        let val = ci[d].0;
        cs = cs.delete(&val).0;
        let idx = cs.find_index(&val).0;
        let upper = cs.at(idx).cloned();
        let lower = if idx >= 1 { cs.at(idx - 1).cloned() } else { None };
        if let Some(u) = upper {
            let dif = u - val;
            let (a, b) = st.query(dif, dif + 1);
            st.update(dif, (a - dif, b - 1));
