...
// Tags: triangles-on-graphs, bitset
fn main() {
    input! {
        n: usize,
        a: [chars; n],
    }
    let mut bs = vec![BitSet::new(3008); n];
    for i in 0..n {
        for j in 0..n {
            if a[i][j] == '1' {
                bs[i].set(j, true);
    ...
    for i in 0..n {
        for j in 0..i {
            if !bs[i].get(j) { continue; }
            let mut tmp = bs[i].clone();
            tmp &= &bs[j];
            ans += tmp.count() as i64;
        }
    }
    println!("{}", ans / 3);
}
