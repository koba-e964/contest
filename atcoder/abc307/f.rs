...
fn main() {
    ...
    const INF: i64 = 1 << 50;
    let mut st = SegTree::new(d, max, -INF);
    for i in 0..d {
        st.update(i, x[i]);
    }
    let mut dist = vec![(d + 1, INF); n];
    let mut que = BinaryHeap::new();
    for &a in &a {
        que.push((Reverse((0, 0)), a));
    }
    while let Some((Reverse(dd), v)) = que.pop() {
        ...
        for &(w, c) in &g[v] {
            if drem >= c {
                que.push((Reverse((dcount, -(drem - c))), w));
            } else {
                let idx = st.max_right(dcount.., &|val| val < c);
                if idx < d {
                    que.push((Reverse((idx + 1, -(x[idx] - c))), w));
                }
            }
        }
    }
