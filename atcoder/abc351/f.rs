...
fn main() {
    ...
    coo.sort(); coo.dedup();
    let m = coo.len();
    let mut stc = SegTree::new(m, |x, y| x + y, 0i64);
    let mut st = SegTree::new(m, |x, y| x + y, 0i64);
    let mut ans = 0;
    for i in (0..n).rev() {
        let a = a[i];
        let idx = coo.binary_search(&a).unwrap();
        let val = st.query(idx..m);
        let cnt = stc.query(idx..m);
        ans += val - cnt * a;
        st.update(idx, st.query(idx..idx + 1) + a);
        stc.update(idx, stc.query(idx..idx + 1) + 1);
    }
    println!("{ans}");
}
