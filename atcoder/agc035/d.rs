...
fn rec(a: &[i64], l: usize, r: usize, lc: i64, rc: i64,
    ...
    let mut mi = i64::max_value();
    for i in l..r {
        let left = rec(a, l, i, lc, lc + rc, memo);
        let right = rec(a, i + 1, r, lc + rc, rc, memo);
        mi = min(mi, left + right + (lc + rc) * a[i]);
    }
    memo.insert(key, mi);
    mi
}
...
fn solve() {
    ...
    puts!("{}\n", rec(&a, 1, n - 1, 1, 1, &mut memo) + a[0] + a[n - 1]);
}
