...
fn calc(n: usize, abc: &[(usize, usize, i32)]) -> (usize, usize) {
    ...
    if e1 == 0 {
        return (n - 1, n - conn + conn * (conn - 1) / 2);
    }
    for &(a, b, c) in abc {
        if c == 1 {
...
fn solve() {
    ...
    input! {
        ...
        abc: [(usize, usize, i32); q],
    }
    let (nec, ma) = calc(n, &abc);
    debugln!("{} {}", nec, ma);
    puts!("{}\n", if nec <= m && m <= ma { "Yes" } else { "No" });
}
