...
fn solve() {
    ...
    let (fac, invfac) = fact_init(0x1000);
    let comb = |n: usize, r: usize| {
        if r > n {
            ModInt::new(0)
        } else {
            fac[n] * invfac[n - r] * invfac[r]
        }
    };
    let mut tot = ModInt::new(0);
    for bits in 0..1 << 4 {
        let dx = [0, 1, 1, 2][bits & 3];
        let dy = [0, 1, 1, 2][bits >> 2];
        if x < dx || y < dy {
            continue;
        }
        let w = x - dx;
        let h = y - dy;
        let mut tmp = comb(w * h, d + l);
    ...
    tot *= (r - x + 1) as i64 * (c - y + 1) as i64;
    // placement of D and L
    tot *= comb(d + l, d);
    puts!("{}\n", tot);
}
