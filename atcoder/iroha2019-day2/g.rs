...
fn solve() {
    ...
    let mut dijk = Dijkstra::new(n * (k + 1));
    for i in 0..k + 1 {
        for &(a, b, c) in &abc {
            dijk.add_edge(i * n + a, i * n + b, c);
            dijk.add_edge(i * n + b, i * n + a, c);
        }
    }
    for i in 0..n {
        let (x, y) = xy[i];
        ...
        for j in 0..k {
            dijk.add_edge(j * n + i, min(j + x, k) * n + i, y);
        }
    }
    const INF: i64 = 1 << 50;
    let ans = dijk.solve(0, 1 << 50)[k * n + n - 1];
