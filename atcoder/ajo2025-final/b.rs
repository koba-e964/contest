...
pub struct FPSOps<M: mod_int::Mod> {
    generator: mod_int::ModInt<M>,
}

impl<M: mod_int::Mod> FPSOps<M> {
    pub fn new(generator: mod_int::ModInt<M>) -> Self {
        FPSOps { generator: generator }
    }
}
...
impl<M: mod_int::Mod> FPSOps<M> {
    ...
    pub fn mul(&self, a: Vec<mod_int::ModInt<M>>, b: Vec<mod_int::ModInt<M>>) -> Vec<mod_int::ModInt<M>> {
        ...
        let zeta = self.generator.pow((M::m() - 1) / p as i64);
...
// Tags: C-recursive, bostan-mori
fn main() {
    let ints = getline().trim().split_whitespace()
        .map(|x| x.parse::<i64>().unwrap())
        .collect::<Vec<_>>();
    let x = ints[1];
    let a: Vec<MInt> = getline().trim().split_whitespace()
        .map(|x| x.parse::<i64>().unwrap().into())
        .collect::<Vec<_>>();
    let n = a.len() / 2 - 1;
    let ops = FPSOps::new(MInt::new(3));
    let (fac, invfac) = fact_init(2 * n + 4);
    let mut p1 = vec![MInt::new(0); n + 2];
    let mut p2 = vec![MInt::new(0); n + 2];
    for i in 0..n + 2 {
        let tmp = fac[n + 1] * invfac[i] * invfac[n + 1 - i];
        let tmp = if i % 2 == 0 {
            tmp
        } else {
            -tmp
        ...
        p1[i] = tmp;
        p2[i] = tmp * MInt::new(2).pow(i as i64);
    }
    let den = ops.mul(p1, p2);
    assert_eq!(den.len(), 2 * n + 3);
    assert_eq!(a.len(), 2 * n + 2);
    let num = ops.mul(den.clone(), a.clone());
    let ans = bostan_mori(&ops, &num[..2 * n + 2], &den, x);
    println!("{ans}");
}
