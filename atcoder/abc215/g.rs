...
fn rec(l: usize, r: usize, polys: &[(Vec<MInt>, Vec<MInt>)]) -> (Vec<MInt>, Vec<MInt>) {
    if l >= r {
        return (vec![1.into()], vec![0.into()]);
    }
    if l + 1 == r {
        return polys[l].clone();
    }
    let mid = (l + r) / 2;
    let (a, b) = rec(l, mid, &polys);
    let (c, d) = rec(mid, r, &polys);
    let ac = conv(a.clone(), c.clone());
    let mut ad = conv(a, d);
    let bc = conv(b, c.clone());
    assert_eq!(ad.len(), bc.len());
    for i in 0..ad.len() {
        ad[i] += bc[i];
    }
    (ac, ad)
...
fn main() {
    ...
    let mut f = vec![0; m];
    for c in c {
        f[coo.binary_search(&c).unwrap()] += 1;
    }
    f.sort();
    let mut polys = vec![];
    for i in 0..m {
        let f = f[i];
        let mut sub00 = vec![MInt::new(0); f + 1];
        let mut sub01 = vec![MInt::new(0); f + 1];
        for j in 1..f + 1 {
            sub01[j] = fac[f] * invfac[j] * invfac[f - j];
            sub00[j] = sub01[j];
        }
        sub00[0] = 1.into();
        polys.push((sub00, sub01));
    }
    let (_, dp1) = rec(0, m, &polys);
    for i in 1..n + 1 {
        puts!("{}\n", dp1[i] * invfac[n] * fac[i] * fac[n - i]);
    }
}
