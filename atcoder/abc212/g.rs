...
fn dfs(v: usize, pe: &[(i64, usize)], cur: i64, phi: i64) -> i64 {
    if v >= pe.len() {
        return cur * phi % MOD;
    }
    let (p, e) = pe[v];
    let mut tot = dfs(v + 1, pe, cur, phi);
    let mut c = cur * (p % MOD) % MOD;
    let mut ph = phi * ((p - 1) % MOD) % MOD;
    for _ in 1..e + 1 {
        tot += dfs(v + 1, pe, c, ph);
        c = c * (p % MOD) % MOD;
        ph = ph * (p % MOD) % MOD;
...
fn main() {
    input!(p: i64);
    let pe = pollard_rho::factorize(p - 1);
    println!("{}", (dfs(0, &pe, 1, 1) + 1) % MOD);
}
