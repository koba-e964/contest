...
fn solve() {
    ...
    input! {
        n: usize,
        m: usize,
        lrx: [(usize1, usize1, i32); m],
    }
    let mut cons = vec![Vec::new(); n + 3];
    for (l, r, x) in lrx {
        cons[r + 3].push((l + 3, x));
    }
    let mut dp = vec![vec![vec![ModInt::new(0); n + 3]; n + 3]; n + 3];
    dp[2][1][0] = ModInt::new(1);
    for i in 3 .. n + 3 {
        for j in 0 .. i - 1 {
            for k in 0 .. j {
                dp[i][j][k] += dp[i - 1][j][k];
                dp[i][i - 1][j] += dp[i - 1][j][k];
                dp[i][i - 1][k] += dp[i - 1][j][k];
            }
        }
        for &(l, x) in cons[i].iter() {
            for j in 0 .. i {
                let kind = 1 + (j >= l) as i32;
                for k in 0 .. j {
                    let kind = kind + (k >= l) as i32;
                    if kind != x {
                        // Doesn't fulfill the constraint. Reset.
                        dp[i][j][k] = ModInt::new(0);
                    }
                }
            }
        }
