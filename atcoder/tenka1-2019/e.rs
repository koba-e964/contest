...
fn solve() {
    ...
    let factt = factorize(a[tt].abs());
    let mut ps: Vec<_> = factt.into_iter().map(|(p, _)| p).collect();
    if a[n] == 0 {
        let pr = is_prime(n + 1);
        for i in 0..n + 1 {
            if pr[i] { ps.push(i as i64); }
        }
    }
    ps.sort(); ps.dedup();
    let mut ans = vec![];
