...
impl MinCostFlow {
    ...
    fn min_cost_flow(&mut self, s: usize, t: usize, mut f: Cap) -> Cost {
        let n = self.n;
        let inf: Cost = std::i64::MAX / 10_000;
        let mut res = 0;
        let h = &mut self.h;
...
// Tags: matroid-intersection, min-cost-flow, small-number-of-edges, segment-tree
fn solve() {
    ...
    const B: usize = 1024;
    let mut mcf = MinCostFlow::new(2 + m + 2 * B - 1);
    for i in 0..m {
        mcf.add_edge(0, 2 + i, 1, 0);
    }
    // segment tree
    const INF: isize = 1 << 20;
    for i in 0..B - 1 {
        for j in 1..=2 {
            mcf.add_edge(2 + m + i, 2 + m + 2 * i + j, INF, 0);
        }
    }
    for i in 0..n {
        mcf.add_edge(2 + m + B - 1 + i, 1, 1, x[i]);
    }
    for i in 0..m {
        let (mut s, mut e) = se[i];
        s += B - 1;
        e += B - 1;
        while s < e {
            if (s & 1) == 0 {
                mcf.add_edge(2 + i, 2 + m + s, 1, 0);
            }
            if (e & 1) == 0 {
                mcf.add_edge(2 + i, 2 + m + e - 1, 1, 0);
            }
            s = s / 2;
            e = (e - 1) / 2;
        }
