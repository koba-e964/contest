...
// Tags: grundy-value, dp, transition-order
fn solve() {
    ...
    let mut pow2 = vec![MInt::new(1); m + 1];
    for i in 1..m + 1 {
        pow2[i] = pow2[i - 1] * 2;
    }
    ...
    let mut indep = vec![MInt::new(0); 1 << n];
    for bits in 1..1 << n {
        if (bits & 3) % 3 != 0 {
            continue;
        }
    ...
    for bits in 1..1 << n {
        for sub in subsets(bits) {
            ...
            let val = dp[sub];
            let mut prod = indep[bits - sub];
            for i in 0..n {
                if (sub & 1 << i) == 0 {
                    continue;
                }
                let free = ((bits - sub) & g[i]).count_ones();
                prod *= pow2[free as usize] - 1;
            }
            for i in 0..n {
                if ((bits - sub) & 1 << i) == 0 {
                    continue;
                }
                let free = (sub & g[i]).count_ones();
                prod *= pow2[free as usize]
            }
            dp[bits] += val * prod;
        }
    }
    puts!("{}\n", MInt::new(2).pow(m as i64) - dp[(1 << n) - 1]);
}
