...
// Tags: fft, smallest-first-convolution
fn main() {
    input! {
    ...
    if k > 0 {
        for i in 0..k - 1 {
            let mut dp = vec![MInt::new(0); a[i + 1] - a[i]];
            let s = a[i + 1] - a[i];
            for j in 0..s {
                dp[j] = fac[s + j] * invfac[2 * j + 1] * invfac[s - j - 1];
            }
            polys.push((Reverse(dp.len()), dp));
        }
        for &s in &[a[0], n - a[k - 1] - 1] {
            let mut dp = vec![MInt::new(0); s + 1];
            for j in 0..s + 1 {
                dp[j] = fac[s + j] * invfac[2 * j] * invfac[s - j];
            }
            polys.push((Reverse(dp.len()), dp));
        }
    } else {
        let mut ans = vec![MInt::new(0); m + 1];
        ans[m] = fac[n + m - 1] * invfac[2 * m - 1] * invfac[n - m];
        polys.push((Reverse(0), ans));
    }
    while polys.len() >= 2 {
        let (_, a) = polys.pop().unwrap();
        let (_, b) = polys.pop().unwrap();
        let c = conv(a, b);
        polys.push((Reverse(c.len()), c));
    }
    let ans = polys.pop().unwrap().1;
    println!("{}", ans[m - k] * fac[m - k]);
}
