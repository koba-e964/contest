...
impl ActionRing for Inversion {
    type T = (i64, i64, i64); // data: (0, 1, inv)
    type U = bool; // action, true |-> 0<->1 involution
    fn biop((x0, x1, xinv): Self::T, (y0, y1, yinv): Self::T) -> Self::T {
        (x0 + y0, x1 + y1, xinv + yinv + x1 * y0)
    }
    fn update((x0, x1, xinv): Self::T, a: Self::U, _height: usize) -> Self::T {
        if a {
            (x1, x0, x0 * x1 - xinv)
        } else {
            (x0, x1, xinv)
...
fn solve() {
    ...
    let mut st = LazySegTree::<Inversion>::new(n);
    for i in 0..n {
        st.dat[st.n - 1 + i] = if a[i] == 0 { (1, 0, 0) } else { (0, 1, 0) };
    }
    for i in (0..st.n - 1).rev() {
        st.dat[i] = Inversion::biop(st.dat[2 * i + 1], st.dat[2 * i + 2]);
    }
    for (t, l, r) in tlr {
        if t == 1 {
            st.update(l, r, true);
        } else {
            let (_, _, inv) = st.query(l, r);
            puts!("{}\n", inv);
        }
    }
}
