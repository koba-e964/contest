...
fn main() {
    ...
    let mut flow = MinCostFlow::new(2 * n + 2);
    const INF: i64 = 1 << 30;
    for i in 0..n {
        for j in 0..n {
            flow.add_edge(2 + i, 2 + n + j, 1, INF - a[i][j]);
        }
        flow.add_edge(0, 2 + i, k as isize, 0);
        flow.add_edge(2 + n + i, 1, k as isize, 0);
    }
    let mut f = 0;
    let mut sum = 0;
    loop {
        let oldflow = flow.clone();
        let cost = flow.min_cost_flow(0, 1, 1);
        if cost >= 0 {
            if cost < INF {
                sum += cost;
                f += 1;
    ...
    puts!("{}\n", f * INF - sum);
    for i in 0..n {
        puts!("{}\n", used[i].iter().cloned().collect::<String>());
    }
}
