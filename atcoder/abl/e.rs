...
impl ActionRing for Affine {
    type T = (ModInt, ModInt); // (data, bias)
    type U = (ModInt, ModInt); // action, (a, b) |-> x |-> ax + b
    fn biop((x, k): Self::T, (y, l): Self::T) -> Self::T {
        (x * l + y, k * l)
    }
    fn update((x, y): Self::T, (a, b): Self::U, _height: usize) -> Self::T {
        (x * a + b * (y - 1), y)
    }
    fn upop(fst: Self::U, snd: Self::U) -> Self::U {
        let (a, b) = fst;
...
fn solve() {
    ...
    input! {
        n: usize, q: usize,
        lrd: [(usize1, usize, i32); q],
    }
    let inv9 = ModInt::new(9).inv();
    let mut st = LazySegTree::<Affine>::new(n);
    for i in st.n - 1..2 * st.n - 1 {
        st.dat[i] = (ModInt::new(1), ModInt::new(10));
    }
    for i in (0..st.n - 1).rev() {
        st.update_node(i);
    }
    st.update(0, n, (ModInt::new(0), ModInt::new(1)));
    for &(l, r, d) in &lrd {
        st.update(l, r, (ModInt::new(0), ModInt::new(d as i64)));
        puts!("{}\n", st.query(0, n).0 * inv9);
    }
}
