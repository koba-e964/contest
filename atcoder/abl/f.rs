...
fn solve() {
    ...
    for f in freq {
        ...
        let mut tbl = vec![ModInt::new(0); f / 2 + 1];
        let mut cur = ModInt::new(1);
        for i in 0..f / 2 + 1 {
            tbl[i] = cur * fac[f] * invfac[i] * invfac[f - 2 * i];
            cur *= inv2;
        }
        que.push((Reverse(tbl.len()), tbl));
    }
    while que.len() >= 2 {
        let (_, v) = que.pop().unwrap();
        let (_, w) = que.pop().unwrap();
        let pr = conv(v, w);
        que.push((Reverse(pr.len()), pr));
    }
    let pr = que.pop().unwrap().1;
    let mut tot = ModInt::new(0);
    let mut cur = inv2.pow(n as i64);
    for i in 0..pr.len() {
        let tmp = cur * fac[2 * n - 2 * i] * invfac[n - i] * pr[i];
        if i % 2 == 0 {
            tot += tmp;
        } else {
            tot -= tmp;
