...
// Tags: perfect-matching, bipartite-matching-with-delta, flow-with-delta
fn main() {
    ...
    let minl = s.iter().map(|x| x.len()).min().unwrap();
    let mut occ = vec![vec![Vec::with_capacity(n); n]; 26];
    for offset in 0..minl {
        for i in 0..26 {
            for j in 0..n {
                occ[i][j].truncate(0);
            }
        ...
        for j in 0..n {
            for k in 0..min(n, s[j].len() - offset) {
                let idx = (s[j][offset + k] as u8 - b'a') as usize;
                occ[idx][j].push(k);
            }
        }
        'outer:
        for letter in 0..26 {
            for j in 0..n {
                if occ[letter][j].is_empty() {
                    continue 'outer;
                }
            }
            let mut din = Dinic::new(2 + 2 * n, 0i32);
            for i in 0..n {
                din.add_edge(0, 2 + i, 1);
                din.add_edge(2 + n + i, 1, 1);
            }
            for j in 0..n {
                for &k in &occ[letter][j] {
                    din.add_edge(2 + j, 2 + n + k, 1);
                }
            }
            let max = din.max_flow(0, 1);
            if max as usize == n {
                puts!("YES\n");
                return;
            }
