...
fn rec<F>(ma_pos: &F, l: usize, r: usize, lwall: bool, rwall: bool, fac: &[MInt], invfac: &[MInt]) -> MInt
    ...
    let pos = ma_pos(l, r);
    let mut ans = rec(ma_pos, l, pos, lwall, false, fac, invfac);
    ans *= rec(ma_pos, pos + 1, r, false, rwall, fac, invfac);
    let i = pos - l;
    let j = r - pos - 1;
    let tmp = match (lwall, rwall) {
        (true, true) => comb(i + j, i),
        (true, false) => comb(i + j + 1, i),
        (false, true) => comb(i + j + 1, j),
        (false, false) => {
...
fn solve() {
    ...
    let pos = |l: usize, r: usize| -> usize {
        let tmp = st.query(l..r);
        inv[tmp]
    };
    println!("{}", rec(&pos, 0, n, true, true, &fac, &invfac));
}
