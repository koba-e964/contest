...
fn main() {
    ...
    for i in 0..m {
        st.set(i, (0.into(), 1.into()));
    }
    st.update(0, m, (1.into(), 1.into()));
    for i in 0..n {
        let val = st.get(a[i]).0;
        if t[i] == 0 {
            let s = st.query(a[i] + 1, m).0;
            st.set(a[i], (val + s, MInt::new(1)));
            st.update(0, a[i], (0.into(), 0.into()));
            st.update(a[i] + 1, m, (0.into(), val));
        } else {
            let s = st.query(0, a[i]).0;
            st.set(a[i], (val + s, MInt::new(1)));
            st.update(0, a[i], (0.into(), val));
            st.update(a[i] + 1, m, (0.into(), 0.into()));
        }
    }
    println!("{}", st.query(0, m).0);
}
