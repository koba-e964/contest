...
fn main() {
    ...
    for i in 1..n + 1 {
        if deg[i] > 0 {
            let mut p = vec![MInt::new(0); deg[i] + 1];
            let mut c = MInt::new(1);
            for j in 0..deg[i] + 1 {
                p[j] = c * fac[deg[i]] * invfac[j] * invfac[deg[i] - j];
                c *= i as i64;
            }
            polys.push((Reverse(deg[i]), p));
        }
    }
    let mut ans = vec![MInt::new(1)];
    while let Some((_, t1)) = polys.pop() {
        let t2 = if let Some((_, t)) = polys.pop() {
            t
        } else {
            ans = t1;
            break;
        ...
        let tmp = conv(t1, t2);
        polys.push((Reverse(tmp.len() - 1), tmp));
    }
