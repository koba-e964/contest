...
fn solve() {
    ...
    let wolf = wolf as i64;
    let fox = fox as i64;
    let mut ans = ModInt::new(0);
    for i in 0 .. n {
        ans += match (s[i], s[i + 1]) {
            ('?', '?') =>
                ModInt::new(2 * wolf * fox)
                * ModInt::new((wolf + fox) * (wolf + fox - 1)).inv(),
            ('?', 'W') =>
                ModInt::new(fox) * ModInt::new(wolf + fox).inv(),
            ('?', 'F') =>
                ModInt::new(wolf) * ModInt::new(wolf + fox).inv(),
            ('W', '?') =>
                ModInt::new(fox) * ModInt::new(wolf + fox).inv(),
            ('F', '?') =>
                ModInt::new(wolf) * ModInt::new(wolf + fox).inv(),
            ('W', 'F') | ('F', 'W') => ModInt::new(1),
            _ => ModInt::new(0),
        }
    }
    for i in 1 .. wolf + 1 {
        ans *= ModInt::new(i).inv();
    }
    for i in fox + 1 .. wolf + fox + 1 {
        ans *= ModInt::new(i);
    }
    puts!("{}\n", ans);
}
