...
fn upper_bound(x: &[i64], a: i64) -> usize {
    match x.binary_search(&a) {
        Ok(a) => a + 1,
        Err(a) => a,
    }
}
...
fn solve() {
    ...
    let mut dpa = vec![ModInt::new(0); n + 1];
    dp[n - 1] = ModInt::new(1);
    dpa[n - 1] = dp[n - 1];
    for i in (0 .. n - 1).rev() {
        let idx1 = upper_bound(&x, x[i] + (f - t));
        let idx2 = upper_bound(&x, x[i] + f);
        let u = idx1 - i - 1;
        let fac = ModInt::new(2).pow(u as i64);
        if idx2 == n {
            dp[i] = ModInt::new(2).pow((n - i - 2) as i64);
        } else {
            dp[i] = (dpa[idx1] - dpa[idx2]) * fac;
        }
        dpa[i] = dpa[i + 1] + dp[i];
        //eprintln!("idx1 = {}, idx2 = {}, i = {}", idx1, idx2, i);
    }
    let tot = dp[0];
    /*
    if true {
