...
struct Updator {}
impl ActionRing for Updator {
    type T = i64;
    type U = Option<i64>;
    fn biop(x: i64, y: i64) -> i64 {
        x ^ y
    }
    fn update(x: i64, y: Option<i64>, h: usize) -> i64 {
        match y {
            None => x,
            Some(y) => {
                if h >= 1 {
    ...
    fn upop(x: Option<i64>, y: Option<i64>) -> Option<i64> {
        match y {
            None => x,
            _ => y
        }
...
fn solve() {
    ...
    used.sort();
    used.dedup();
    let at = |x| used.binary_search(&x).unwrap();
    let m = used.len();

    let mut st = LazySegTree::<Updator>::new(m);
    let mut init = vec![0; m];
    for &a in &a {
        init[at(a)] ^= 1;
    }
    for i in 0..m {
        let val = if init[i] == 1 { used[i] << 1 | 1 } else { 0 };
        st.update(i, i + 1, Some(val));
    }
    for &(l, r, x) in &lrx {
        let (l, r) = (at(l), at(r + 1));
        let val = st.query(l, r);
        puts!("{}\n", val >> 1);
        st.update(l, r, Some(0));
        let xidx = at(x);
        let newval = if (val & 1) == 1 { x << 1 | 1 } else { 0 }
        ^ st.query(xidx, xidx + 1);
        st.update(xidx, xidx + 1, Some(newval));
    }
}
