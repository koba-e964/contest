...
fn main() {
    ...
    for i in 0..n {
        ...
        if midx % 2 == 1 {
            ...
            for j in 0..l / 2 {
                let x = pts[i][2 * j].0;
                let y0 = pts[i][2 * j].1;
                let y1 = pts[i][2 * j + 1].1;
                coord.push(y0);
                coord.push(y1);
                ev.push((2 * x, y1, y0, 0, 0));
            }
        } else {
            // clockwise
            for j in 0..l / 2 {
                ...
                coord.push(y0);
                coord.push(y1);
                ev.push((2 * x, y0, y1, 0, 0));
            }
        }
    }
    for i in 0..q {
        let (x, y) = xy[i];
        ev.push((2 * x + 1, y, 0, 1, i));
        coord.push(y);
    }
    ...
    for &(_x, y0, y1, kind, idx) in &ev {
        let k0 = coord.binary_search(&y0).unwrap();
        if kind == 0 {
            let k1 = coord.binary_search(&y1).unwrap();
            if y0 < y1 {
                st.update(k0, k1, (1, 1));
            } else {
                st.update(k1, k0, (1, -1));
