...
fn calc(n: usize, pclr: &[(usize, char, usize, usize)]) -> bool {
...
fn solve() {
    ...
    for i in 0..lim {
        let (p, _, l, r) = pclr[i];
        coo.extend_from_slice(&[p, l, r]);
    }
    coo.sort(); coo.dedup();
    let pclr: Vec<_> = pclr[..lim].iter().map(|(p, c, l, r)| {
