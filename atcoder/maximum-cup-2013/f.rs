...
fn solve() {
    input! {
        n: usize, m: usize,
        kxy: [(String, i64, i64); n],
        mxy: [(String, u64, i64, i64); m],
    }
    let mut mxy = mxy;
    mxy.sort_by_key(|x| x.1);
    mxy.reverse();
    let f = std::cmp::min(n, m);
    let x = mxy[f - 1].1;
    let mut tie = 0;
    for i in 0..f {
        if mxy[i].1 == x { tie += 1; }
    ...
    let big = 1 << 30;
    let mut mcf = MinCostFlow::new(2 + n + m + n);
    for i in 0..n {
        mcf.add_edge(0, 2 + i, 1, 0);
        for j in 0..m {
            let dist = (kxy[i].1 - mxy[j].2).abs() + (kxy[i].2 - mxy[j].3).abs();
            if mxy[j].1 >= x {
                mcf.add_edge(2 + i, 2 + n + j, 1, if mxy[j].1 == x { big } else { 0 } + dist);
            }
        }
        for j in 0..n {
            if x == 0 {
                mcf.add_edge(2 + i, 2 + n + m + j, 1, big);
            }
        }
    }
    for i in 0..m + n {
    ...
    let ans = mcf.min_cost_flow(0, 1, f as isize);
    println!("{}", ans - tie * big);
}
