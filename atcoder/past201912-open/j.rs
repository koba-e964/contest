    ...
    for i in 0..h {
        for j in 0..w {
            for &(dx, dy) in &dxy {
                ...
                let nx = nx as usize;
                let ny = ny as usize;
                dijk.add_edge(i * w + j, nx * w + ny, a[nx][ny]);
            }
        }
    }
    const INF: i64 = 1 << 50;
    let mut mi = INF;
    let dist1 = dijk.solve((h - 1) * w, INF);
    let dist2 = dijk.solve((h - 1) * w + w - 1, INF);
    let dist3 = dijk.solve(w - 1, INF);
    for i in 0..h * w {
        mi = min(mi, dist1[i] + dist2[i] + dist3[i] - 2 * a[i / w][i % w]);
    }
    puts!("{}\n", mi);
}
