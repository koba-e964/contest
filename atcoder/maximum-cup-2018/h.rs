...
fn solve() {
    ...
    while pass - fail > 1 {
        ...
        let mut nec = vec![];
        for i in 0..k - 1 {
            if dist[a[i]][a[i + 1]] > mid {
                nec.push(a[i + 1]);
            }
        }
        let mut din = Dinic::new(2 + q + nec.len(), 0i64);
        for i in 0..q {
            din.add_edge(2 + i, 1, 1);
        }
        for i in 0..nec.len() {
            din.add_edge(0, 2 + q + i, 1);
        }
        for i in 0..nec.len() {
            for j in 0..q {
                if dist[nec[i]][b[j]] <= mid {
                    din.add_edge(2 + q + i, 2 + j, 1);
                }
            }
        }
        let (x, _) = din.max_flow(0, 1);
        if x as usize >= nec.len() {
            pass = mid;
        } else {
            fail = mid;
        }
