...
fn solve() {
    ...
    let mut dijk = Dijkstra::new(2 * r * c);
    for i in 0..r {
        for j in 0..c - 1 {
            let v = i * c + j;
            dijk.add_edge(v, v + 1, a[i][j]);
            dijk.add_edge(v + 1, v, a[i][j]);
        }
    }
    for i in 0..r - 1 {
        for j in 0..c {
            let v = i * c + j;
            dijk.add_edge(v, v + c, b[i][j]);
            dijk.add_edge(v + c + r * c, v + r * c, 1);
        }
    }
    for i in 0..r * c {
        dijk.add_edge(i, i + r * c, 1);
        dijk.add_edge(i + r * c, i, 0);
    }
