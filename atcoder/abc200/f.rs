...
fn solve() {
    ...
    let mut t0 = vec![vec![MInt::new(0); 4]; 4];
    for i in 0..s.len() {
        let mut now = vec![vec![MInt::new(0); 4]; 4];
        match s[i] {
            '0' => {
                now[0][0] += 1;
                now[1][0] += 1;
        ...
        if i == 0 {
            ...
            trans = squmul(&trans, &now);
        }
    }
    let mt = squmul(&t0, &trans);
    eprintln!("trans = {:?}", mt);
    trans = squmul(&trans, &squpow(&mt, k - 1));
    eprintln!("trans = {:?}", mt);
    let mut sum = MInt::new(0);
    if s[0] != '0' {
        sum += trans[1][2] + trans[1][0];
    }
    if s[0] != '1' {
        sum += trans[0][2] + trans[0][1];
    }
    puts!("{}\n", sum * MInt::new(2).inv());
}
