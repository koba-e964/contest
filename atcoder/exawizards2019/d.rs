...
fn solve() {
    ...
    let mut inv = vec![ModInt::new(0); W];
    for i in 1..W {
        inv[i] = invfac[i] * fac[i - 1];
    }
    inv[0] = ModInt::new(1); // hack
    let mut s = s;
    s.sort();
    s.reverse();
    let mut dp = vec![vec![ModInt::new(0); x + 1]; n + 1];
    let mut sum = vec![ModInt::new(0); x + 1];
    dp[0][x] = inv[n];
    sum[x] = dp[0][x];
    for i in 0..n {
        for j in 1..x + 1 {
            dp[i + 1][j % s[i]] += sum[j] * inv[n - i - 1];
        }
        for j in 1..x + 1 {
            sum[j] += dp[i + 1][j];
        }
    ...
    for j in 1..x + 1 {
        ans += dp[n][j] * ModInt::new(j as i64);
    }
    puts!("{}\n", ans * fac[n]);
}
