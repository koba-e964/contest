...
fn calc(f: &[usize], a: MInt, m: usize, fac: &[MInt], invfac: &[MInt]) -> MInt {
    let mut que = BinaryHeap::new();
    for i in 1..11 {
        let mut v = vec![MInt::new(0); i * f[i] + 1];
        let mut cur = MInt::new(1);
        let x = a;
        for j in 0..f[i] + 1 {
            v[i * j] = cur * fac[f[i]] * invfac[j] * invfac[f[i] - j];
            cur *= x;
        }
        que.push((Reverse(f[i] + 1), v));
    }
    let mut ans = vec![];
    while let Some((_, p1)) = que.pop() {
        if let Some((_, p2)) = que.pop() {
            let p = conv(p1, p2);
            que.push((Reverse(p.len()), p));
            continue;
        }
        ans = p1;
        break;
...
fn main() {
    ...
    let x = calc(&f, MInt::new(1), m, &fac, &invfac);
    let y = calc(&f, -MInt::new(1), m, &fac, &invfac);
    println!("{}", (x - y) * MInt::new(2).inv());
}
