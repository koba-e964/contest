...
// Solved with hints
// x[i] や y[i] を order encoding する。
// フローに帰着させる都合上 0 => 0, 1 => 1 という制約しか追加できないことに注意。
// ref: https://zenn.dev/semiexp/books/e559bbd5ab00a2/viewer/4e996c
// Tags: order-encoding, sat, minimum-cut, submodular-minimization
fn solve() {
    input! {
        n: usize, m: usize,
        x: [usize; n],
    ...
    const INF: i64 = 1 << 50;
    const W: usize = 500;
    let mut sub = SubmodMin::new(m + 2 * n * W);
    for i in 0..n {
        for j in 0..W - 1 {
            // 0..01..1
            sub.add2(m + 2 * i * W + j, m + 2 * i * W + j + 1, [[0, 0], [INF, 0]]);
            // 1..10..0
            sub.add2(m + 2 * i * W + W + j, m + 2 * i * W + W + j + 1, [[0, INF], [0, 0]]);
        }
        for j in 0..W {
            sub.add1(m + 2 * i * W + j, [1, 0]);
            sub.add1(m + 2 * i * W + W + j, [0, 1]);
        }
    }
    for i in 0..n {
        // [i][x[i] - 1] = 0
        sub.add1(m + 2 * i * W + x[i] - 1, [0, INF]);
        // [i][y[i] - 1] = 1
        sub.add1(m + 2 * i * W + W + y[i] - 1, [INF, 0]);
    }
    for i in 0..m {
        for j in 0..n {
            // [i] = 0 => [j][a[i][j] - 1] = 0
            sub.add2(i, m + 2 * j * W + a[i][j] - 1, [[0, INF], [0, 0]]);
            // [i] = 1 => [j][a[i][j] - 1] = 1
            sub.add2(i, m + 2 * j * W + W + a[i][j] - 1, [[0, 0], [INF, 0]]);
        }
    }
    println!("{}", sub.calc());
}
