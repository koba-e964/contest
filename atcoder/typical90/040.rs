          ...
          T: std::ops::SubAssign,
{
    ...
    pub fn add_edge(&mut self, from: usize, to: usize, cap: T) {
        let added_from = Edge { to: to, cap: cap, rev: self.graph[to].len() };
...
fn main() {
    ...
    const INF: i64 = 1 << 40;
    let mut din = Dinic::new(n + 2, 0i64);
    for i in 0..n {
        din.add_edge(0, 2 + i, w);
        din.add_edge(2 + i, 1, a[i]);
        for &j in &c[i] { din.add_edge(2 + i, 2 + j, INF) }
    }
    let mut ans = -din.max_flow(0, 1).0;
    for a in a { ans += a; }
    println!("{}", ans);
}
