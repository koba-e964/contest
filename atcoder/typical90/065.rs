...
fn main() {
    ...
    let (fac, invfac) = fact_init(a + b + c);
    let (x, y, z) = (k - y, k - z, k - x);
    let mut dp1 = vec![MInt::new(0); a + 1];
    let mut dp2 = vec![MInt::new(0); b + 1];
    for i in x..a + 1 {
        dp1[i] = fac[a] * invfac[i] * invfac[a - i];
    }
    for i in y..b + 1 {
        dp2[i] = fac[b] * invfac[i] * invfac[b - i];
    }
    let dp12 = conv(dp1, dp2);
    let mut tot = MInt::new(0);
    for i in z..min(k, c) + 1 {
        let to = k - i;
        if to < dp12.len() {
            tot += dp12[to] * fac[c] * invfac[i] * invfac[c - i];
        }
    }
    println!("{}", tot);
}
