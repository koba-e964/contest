...
fn solve() {
    ...
    let big: i64 = 1 << 30;
    let bigcap: isize = 1000;
    let mut mcf = MinCostFlow::new(2 + n + m);
    mcf.add_edge(0, 1, bigcap, big);
    for i in 0..n {
        mcf.add_edge(0, 2 + i, a[i], 0);
        for j in 0..m {
            mcf.add_edge(2 + i, 2 + n + j, bigcap, big - c[i][j]);
        }
    }
    for j in 0..m {
        mcf.add_edge(2 + n + j, 1, b[j], 0);
    }
    let ans = mcf.min_cost_flow(0, 1, bigcap);
    println!("{}", bigcap as i64 * big - ans);
}
