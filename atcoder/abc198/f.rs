...
fn calc(n: i64, conn: Vec<usize>) -> MInt {
    let mut mat = vec![vec![MInt::new(0); 7]; 7];
    let k = conn.len();
    let mut idx = vec![0; k + 1];
    for i in 0..k {
        idx[i + 1] = idx[i] + conn[i];
    }
    for i in 0..k {
        for j in idx[i]..idx[i + 1] {
            mat[j][j + 1] += 1;
        }
        mat[idx[i + 1]][idx[i] + 1] += 1;
    }
    let pw = squpow(&mat, n);
    pw[0][6]
}
