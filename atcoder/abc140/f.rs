...
fn naive(n: usize, mut s: Vec<i64>) -> bool {
    s.sort();
    let mut ans = vec![0; 1 << n];
    for bc in 0..n + 1 {
        for bits in 0..1usize << n {
            if bits.count_ones() as usize == bc {
                ans[bits] = s.pop().expect("2^n = 2^n");
            }
        }
    }
    for i in (0..n).rev() {
        for bits in 0..1 << i {
            if ans[bits] <= ans[bits | 1 << i] {
                return false;
            }
        }
    }
...
fn correct(n: usize, s: Vec<i64>) -> bool {
    let mut tr = TwoThreeTree::new();
    for i in 0..1 << n {
        tr = tr.insert((s[i], i + 1));
    }
    let &ma = tr.at((1 << n) - 1).unwrap();
    tr = tr.delete(&ma).0;
    let mut cur = vec![ma.0];
    for _d in 0..n {
        let mut nxt = vec![];
        // greedily pick
        for &c in &cur {
            let idx = tr.find_index(&(c, 0)).0;
            if idx > 0 {
                let &val = tr.at(idx - 1).unwrap();
                tr = tr.delete(&val).0;
                nxt.push(val.0);
            } else {
                return false;
...
fn find_counterexamples() {
    ...
    let mut x: u64 = 1511;
    for _ in 0..10000 {
        let mut dat = vec![0; 1 << n];
        for i in 0..1 << n {
            x = x.wrapping_mul(11515153).wrapping_add(11);
            let val = (x / 63333) % ma;
            dat[i] = val as i64;
        }
        let ans_naive = naive(n, dat.clone());
        let ans_correct = correct(n, dat.clone());
        if ans_naive != ans_correct {
            debugln!("s = {:?} naive = {} correct = {}", dat, ans_naive, ans_correct);
        }
    }
}
