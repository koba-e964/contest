...
fn solve() {
    ...
    input! {
        x: usize, y: usize, z: usize, n: usize, m: usize,
        s: usize1, t: usize1,
        ab: [(usize1, usize1); n],
        cd: [(usize1, usize1); m],
    }
    let mut dijk = Dijkstra::new(x + n + y + m + z);
    for i in 0..n {
        let (a, b) = ab[i];
        dijk.add_edge(x + i, a, 0);
        dijk.add_edge(a, x + i, 1);
        dijk.add_edge(x + i, x + n + b, 0);
        dijk.add_edge(x + n + b, x + i, 1);
    }
    for i in 0..m {
        let (c, d) = cd[i];
        dijk.add_edge(x + n + y + i, x + n + c, 0);
        dijk.add_edge(x + n + c, x + n + y + i, 1);
        dijk.add_edge(x + n + y + i, x + n + y + m + d, 0);
        dijk.add_edge(x + n + y + m + d, x + n + y + i, 1);
    }
    const INF: i64 = 1 << 60;
    let dist = dijk.solve(x + s, INF);
    let d = dist[x + n + y + t];
    if d >= INF {
        puts!("-1\n");
    } else {
