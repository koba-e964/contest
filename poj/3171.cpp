...
const int D = 86401;
ll inf = 1e16;
vector<pair<int, ll> > pool[D];
int main(void){
  cin.tie(0);
  ios::sync_with_stdio(false);
  int n, m, e;
  cin >> n >> m >> e;
  SegTree<ll, min_func> st(D, min_func(), inf);
  REP(i, 0, n) {
    int t1, t2;
    ll s;
    cin >> t1 >> t2 >> s;
    t2++;
    pool[t2].push_back(pair<int, ll>(t1, s));
  }
  st.update(m, 0);
  REP(i, m, e + 1) {
    ll mi = inf;
    REP(j, 0, pool[i + 1].size()) {
      pair<int, ll> dat = pool[i + 1][j];
      mi = min(mi, st.query(dat.first, i) + dat.second);
    }
    st.update(i + 1, mi);
  }
  ll res = st.query(e + 1, e + 1);
  cout << (res == inf ? -1 : res) << "\n";
}
