...
int main(void) {
int n;cin>>n;
vector<pair<double, int> > vol(n);
rep(i, n) {
double r, h;
cin >> r >> h;
vol[i] = make_pair(r * r * h, -i);
}
sort(vol.begin(), vol.end());
SegTree<double, pmax> st(n, pmax(), 0.0);
rep(i, n) {
int idx = -vol[i].second;
double t = vol[i].first;
double res = st.query(0, idx - 1) + t;
st.update(idx, res);
}
printf("%.15f\n", 3.1415926535897932384626 * st.query(0, n - 1));
}
