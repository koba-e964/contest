...
// 1 <= ... <= a
fn calc(a: i64, k: usize) -> ModInt {
    let mut inex = vec![ModInt::new(0); 1 << 10];
    let mut dig = [0; 18];
    {
        let mut v = a;
        for i in 0 .. 18 {
    ...
    for bits in 0 .. 1 << 10 {
        let mut dp = [[[ModInt::new(0); 2]; 3]; 19]; // [pos][lt? eq? gt?][trailing-zeros?]
        let mut dp2 = [[[ModInt::new(0); 2]; 3]; 19]; // [pos][lt? eq? gt?][trailing-zeros?]
        dp[0][1][1] = ModInt::new(1);
        let mut pow10 = ModInt::new(1);
        for i in 0 .. 18 {
            for d in 0 .. 10 {
                if (bits & 1 << d) == 0 { continue; }
                for x in 0 .. 3 {
                    let sndidx = if d == dig[i] {
                        x
                    } else if d < dig[i] { 0 } else { 2 };
                    for y in 0 .. 2 {
                        dp[i + 1][sndidx][if d == 0 { 1 } else { 0 }]
                            += dp[i][x][y];
                        dp2[i + 1][sndidx][if d == 0 { 1 } else { 0 }]
                            += dp2[i][x][y] +
                            ModInt::new(d) * pow10 * dp[i][x][y];
                    }
                }
            }
            inex[bits] += dp2[i + 1][0][0] + dp2[i + 1][1][0];
    ...
    for i in 0 .. 10 {
        for bits in 0 .. 1 << 10 {
            if (bits & 1 << i) == 0 {
                inex[bits | 1 << i] -= inex[bits];
            }
        }
    }
    let mut tot = ModInt::new(0);
    for bits in 0 .. 1usize << 10 {
