...
fn calc(mut a: Vec<usize>, lcp: &LCP, len: usize) -> i64 {
    ...
    for i in 0 .. n - 1 {
        if a[i] == a[i + 1] {
            diff[i] = (len - a[i]) as i64;
        } else {
            diff[i] = lcp.get_lcp(a[i], a[i + 1]) as i64;
        }
    }
    diff[n - 1] = -1;
    let mut tot = 0;
    ...
    st.push((0, -2));
    for i in 0 .. n {
        while st.last().unwrap().1 >= diff[i] {
            let idx = st.len() - 2;
            let (center_idx, val) = st[idx + 1];
            let l = st[idx].0;
            let r = i + 1;
            tot += (r - center_idx) as i64 * (center_idx - l) as i64 * val as i64;
            st.pop();
        }
        st.push((i + 1, diff[i]));
    }
