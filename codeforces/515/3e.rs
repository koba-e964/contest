...
fn solve() {
    ...
    a.reverse();
    b.reverse();
    let a: Vec<_> = a.into_iter().map(|v| if v == '0' { 0 } else { 1 }).collect();
    let b: Vec<_> = b.into_iter().map(|v| if v == '0' { 0 } else { 1 }).collect();
    let mut cnt = vec![0; n];
    let mut c = 0;
    for i in (0 .. max(n, m)).rev() {
        if i < m {
            c += b[i];
        }
        if i < n {
