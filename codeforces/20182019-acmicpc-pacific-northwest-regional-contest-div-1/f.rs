...
impl ActionRing for SumAction {
    type T = i64;
    type U = (i64, i32); // action
    fn biop(x:  Self::T, y: Self::T) -> Self::T {
        x + y
    }
    fn update(x: Self::T, a: Self::U, height: usize) -> Self::T {
        a.1 as i64 * x + (a.0 << height)
    }
    fn upop(a: Self::U, b: Self::U) -> Self::U {
        (a.0 * b.1 as i64 + b.0, a.1 * b.1)
    }
    fn e() -> Self::T {
        0
    }
...
fn solve() {
    ...
    input! {
        n: usize,
        rect: [(i64, i64, i64, i64); n],
    }
    let mut coord = vec![];
    for &(x1, _, x2, _) in &rect {
        coord.push(x1);
        coord.push(x2);
    }
    coord.sort();
    ...
    for &(x1, y1, x2, y2) in &rect {
        let x1 = coord.binary_search(&x1).unwrap();
        let x2 = coord.binary_search(&x2).unwrap();
        events.push((y1, x1, x2));
        events.push((y2, x1, x2));
    }
    events.sort();
    let mut st = LazySegTree::<SumAction>::new(m);
    let all = coord[m - 1] - coord[0];
    for i in 0..m - 1 {
        st.update(i, i + 1, (-coord[i + 1] + coord[i], 0));
    }
    let mut tot = 0;
    let mut lasty = 0;
    for &(y, x1, x2) in &events {
        let odd = (st.query(0, m - 1) + all) / 2;
        // eprintln!("odd = {}, lasty = {}, y = {}", odd, lasty, y);
        tot += odd * (y - lasty);
        st.update(x1, x2, (0, -1));
        lasty = y;
    }
