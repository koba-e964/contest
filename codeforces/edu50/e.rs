...
fn det<T: std::ops::Mul<Output = T> + std::ops::Sub<Output = T>>
    ((x, y): (T, T), (z, w): (T, T)) -> T {
    x * w - y * z
}
...
fn intersect((p11, p12): ((i64, i64), (i64, i64)), (p21, p22): ((i64, i64), (i64, i64)))
    ...
    if ccw(p11, p12, p21) * ccw(p11, p12, p22) <= 0
        && ccw(p21, p22, p11) * ccw(p21, p22, p12) <= 0 {
            let (a00, a10) = (p12.0 - p11.0, p12.1 - p11.1);
            let a1 = (p22.0 - p21.0, p22.1 - p21.1);
            let v = (p21.0 - p11.0, p21.1 - p11.1);
            let (s_num, s_den) = (det(v, a1), det((a00, a10), a1));
