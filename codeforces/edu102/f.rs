...
fn solve() {
    ...
    for i in 0..n {
        din.add_edge(0, 1 + i, BIAS);
        din.add_edge(1 + i, n + 1, BIAS - b[i]);
    }
    let mut last = vec![n + 1; 101];
    for i in (0..n).rev() {
        for j in 1..100 / a[i] + 1 {
            if last[j * a[i]] < n {
                din.add_edge(1 + last[j * a[i]], 1 + i, INF);
            }
        }
        last[a[i]] = i;
    }
    let (ans, _) = din.max_flow(0, n + 1);
    puts!("{}\n", -ans + (n as i64) * BIAS);
}
