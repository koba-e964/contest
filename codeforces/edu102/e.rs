...
fn solve() {
    ...
    let mut dijk = Dijkstra::new(4 * n);
    for &(a, b, c) in &abc {
        for j in 0..2 {
            for k in 0..2 {
                dijk.add_edge(a + 2 * n * j + n * k, b + 2 * n * j + n * k, c);
                dijk.add_edge(b + 2 * n * j + n * k, a + 2 * n * j + n * k, c);
            }
            dijk.add_edge(a + 2 * n * j, b + 2 * n * j + n, 0);
            dijk.add_edge(b + 2 * n * j, a + 2 * n * j + n, 0);
        }
        for j in 0..2 {
            dijk.add_edge(a + n * j, b + n * j + 2 * n, 2 * c);
            dijk.add_edge(b + n * j, a + n * j + 2 * n, 2 * c);
        }
        dijk.add_edge(a, b + 3 * n, c);
        dijk.add_edge(b, a + 3 * n, c);
    }
    let sol = dijk.solve(0, 1 << 50);
    let mut ans = vec![0; n - 1];
    for i in 1..n {
        ans[i - 1] = min(sol[2 * n + i], sol[3 * n + i]);
    }
    putvec!(ans);
}
