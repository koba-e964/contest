...
void prog(SegTree<int, xori> &uniq, map<int, int> &occur, int cur) {
  // manipulate a[cur]
  if (occur.count(a[cur])) {
    int old = occur[a[cur]];
    uniq.update(old, 0);
  }
  occur[a[cur]] = cur;
  uniq.update(cur, a[cur]);
}
...
int main(void){
  ...
  SegTree<int, xori> st(n, xori(), 0);
  SegTree<int, xori> uniq(n, xori(), 0);
  REP(i, 0, n) {
    st.update(i, a[i]);
  }
  int m;
  ...
  REP(i, 0, m) {
    ...
    scanf("%d%d", &l, &r);
    l--, r--;
    query.push_back(pair<PI, int>(PI(r, l), i));
  }
  ...
  REP(i, 0, m) {
    ...
    int l = q.second;
    int r = q.first;
    res[idx] = uniq.query(l, r) ^ st.query(l, r);
  }
  REP(i, 0, m) {
    printf("%d\n", res[i]);
  }    
