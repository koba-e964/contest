...
int main(void) {
  ...
  REP(i, 0, n - 1) {
    ...
    int pos = 0;
    for (; pos < min(l1, l2); ++pos) {
      int c1 = words[i][pos];
      int c2 = words[i + 1][pos];
      if (c1 != c2) {
	if (c1 < c2) {
	  cons.push_back(PI(c1, -c2));
	} else {
	  // v[c1] = true && v[c2] = false
	  cons.push_back(PI(c1, c1));
	  cons.push_back(PI(-c2, -c2));
	}
	break;
      }
    }
    if (pos == min(l1, l2)) {
      if (l1 > l2) {
	// impossible
	cons.push_back(PI(1, 1));
	cons.push_back(PI(-1, -1));
	break;
      }
    }
  }
  vector<int> sol = two_sat(m, cons);
  if (sol.size() == 0) {
    cout << "No\n";
    return 0;
  }
