...
fn solve() {
    ...
    let mut prob = vec![vec![MInt::new(0); n]; n];
    for i in 0..n {
        for j in 0..n {
            prob[i][j] = MInt::new(a[i]) * MInt::new(a[i] + a[j]).inv();
        }
    }
    let mut scc = vec![MInt::new(0); 1 << n];
    let mut bord = vec![MInt::new(0); 1 << n];
    for bits in 1..1 << n {
    ...
    for bits in 0usize..1 << n {
        ...
        tot += scc[bits] * bord[bits] * bc as i64;
    }
    println!("{}", tot);
}
