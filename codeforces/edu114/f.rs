...
fn dfs(v: usize, vis: &mut [bool], g: &[Vec<usize>], revg: &[Vec<usize>]) -> i32 {
    if g[v].len() >= 2 || revg[v].len() >= 2 {
        return -1;
    }
    if vis[v] {
        return -1;
...
fn solve() {
    ...
    for i in 0..n {
        for j in 0..a[i].len() - 1 {
            g[a[i][j]].push(a[i][j + 1]);
        }
    }
    for i in 0..k {
        g[i].sort_unstable(); g[i].dedup();
    }
    let mut revg = vec![vec![]; k];
    for i in 0..k {
        for &w in &g[i] {
            revg[w].push(i);
        }
    ...
    let mut coef = vec![MInt::new(0); p];
    coef[0] = 1.into();
    for i in 0..k {
        if !vis[i] && revg[i].is_empty() {
            let kind = dfs(i, &mut vis, &g, &revg);
            if kind >= 0 {
                coef[kind as usize] -= 1;
            }
        }
    }
    let ans = fps_inv(&coef, 3.into());
    println!("{}", ans[m]);
}
