...
// Tags: many-intervals, dp, diagonal-cumulative-sum, unusual-cumulative-sum, inclusion-exclusion-principle
fn main() {
    input! {
        n: i64, k: usize,
        m: usize,
    ...
    let mut acc_diag = vec![vec![MInt::new(0); k + 1]; m + 1];
    acc[m][k] = 1.into();
    acc_diag[m][0] = 1.into();
    for i in (0..m).rev() {
        let (l, r) = lr[i];
        ...
        if i + 1 < idx {
            ...
            assert!(lnxt < r);
            for j in 0..k - l + 1 {
                me[j + l] += acc_diag[idx][j];
                me[j + l] -= acc_diag[i + 1][j];
            }
        }
        for j in l..k + 1 {
            acc[i][j] = acc[i + 1][j] + me[j];
            acc_diag[i][j - l] = acc_diag[i + 1][j - l] + me[j];
            dp[i][j] = me[j];
        }
    }
