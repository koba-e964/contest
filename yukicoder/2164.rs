...
// \sum_i C(n, i) * C(m, i+d) = C(n + m, m - d)
fn f(n: usize, m: usize, d: isize, fac: &[MInt], invfac: &[MInt]) -> MInt {
    if d > m as isize || d < -(n as isize) {
        return 0.into();
    }
    let x = (m as isize - d) as usize;
    fac[n + m] * invfac[x] * invfac[n + m - x]
}
...
fn convolve(a: &[Vec<MInt>]) -> Vec<MInt> {
    if a.is_empty() {
        return vec![MInt::new(1)];
    }
    if a.len() == 1 {
        return a[0].clone();
    }
    let frm = convolve(&a[..a.len() / 2]);
    let lat = convolve(&a[a.len() / 2..]);
    conv(frm, lat)
}

// https://yukicoder.me/problems/no/2164 (3.5)
// TLE が取れない。
// f の計算後に panic! を入れて提出したところすべて RE になったので、ボトルネックは畳み込みと思われる。
// 畳み込みに FFT を使い AC。
fn main() {
    input! {
        n: usize, m: usize,
    ...
    const BIAS: usize = 300;
    const W: usize = 601;
    let (fac, invfac) = fact_init(W);
    let mut grouped = vec![vec![MInt::new(1); W]; m];
    for i in 0..n {
        let rem = i % m;
        for j in 0..W {
            grouped[rem][j] *= f(a[i], b[i], j as isize - BIAS as isize, &fac, &invfac);
        }
    }
    let frm = convolve(&grouped[..m / 2]);
    let lat = convolve(&grouped[m / 2..]);
    let mut ans = MInt::new(0);
    for i in 0..frm.len() {
        let latidx = m * BIAS - i;
        ans += frm[i] * lat[latidx];
    }
    println!("{ans}");
}
