...
// https://yukicoder.me/problems/no/1441 (3)
// 有効な区間の左端を 1、それ以外を 0 としてセグ木に載せる。
fn main() {
    ...
    for (t, l, r) in tlr {
        if t == 1 {
            for _ in l + 1..r {
                let to = st.max_right(0, &|x| x <= l + 1);
                st.update(to, 0);
            }
        } else {
            let lto = st.max_right(0, &|x| x <= l);
            let rto = st.max_right(0, &|x| x <= r);
            puts!("{}\n", acc[rto] - acc[lto]);
        }
    }
}
