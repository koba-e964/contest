...
fn main() {
    ...
    let mut ffreq = HashMap::new();
    for i in 0..m {
        if freq[i] > 0 {
            *ffreq.entry(freq[i]).or_insert(0) += 1;
        }
    }
    let ffreq: Vec<_> = ffreq.into_iter().collect();
    ...
    for i in 0..n + 1 {
        ...
        for &(k, v) in &ffreq {
            ...
            tmp *= (fac[i] * invfac[i - k]).pow(v as i64);
        }
        f[i] = tmp * invfac[i];
    }
    let fps = FPSOps::new(3.into());
    let mut einv = vec![MInt::new(0); n + 1];
    for i in 0..n + 1 {
        einv[i] = invfac[i];
        if i % 2 != 0 {
            einv[i] = -einv[i];
