...
// https://yukicoder.me/problems/no/3201 (3.5)
// 0-1 変数の劣モジュラー最小化に帰着できる。これは最大フローに帰着できる。
fn solve() {
    input! {
        n: usize,
        p: [i64; n],
        ...
        uv: [(usize1, usize1); m],
        k: usize,
        abs: [(usize1, usize1, i64); k],
    }
    let mut submodmin = SubmodMin::new(n);
    for i in 0..n {
        submodmin.add1(i, [0, -p[i]]);
    }
    for &(u, v) in &uv {
        submodmin.add2(u, v, [[0, INF], [0, 0]]);
    }
    for &(a, b, s) in &abs {
        submodmin.add2(a, b, [[0, 0], [0, -s]]);
    }
    let ans = -submodmin.calc();
    println!("{ans}");
}
