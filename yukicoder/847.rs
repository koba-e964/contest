...
fn dfs(idx: usize, m: i64, fac: &[(i64, i64)], memo: &mut HashMap<(usize, i64), i64>) -> i64 {
    if let Some(&val) = memo.get(&(idx, m)) {
        return val;
    }
    if idx >= fac.len() {
        return min(m, 1);
    ...
    let (p, mut e) = fac[idx];
    while v > 0 && e >= 0 {
        sum += dfs(idx + 1, v, fac, memo);
        v /= p;
        e -= 1;
    }
    memo.insert((idx, m), sum);
    sum
}
...
fn solve() {
    ...
    let fac: Vec<_> = pollard_rho::factorize(n)
        .into_iter()
        .map(|(p, e)| (p, e as i64 * k))
        .collect();
    let mut memo = HashMap::new();
    puts!("{}\n", dfs(0, m, &fac, &mut memo));
}
