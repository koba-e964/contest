...
// https://yukicoder.me/problems/no/2670 (4)
// The author read the editorial before implementing this.
// Solved with hints
// ref:
// - https://sugarknri.hatenablog.com/entry/2022/06/08/180533
// - https://koba-e964.hatenablog.com/entry/2024/04/15/230008
// - https://tsuchi.hateblo.jp/entry/2024/03/09/000125
// Similar problems: https://yukicoder.me/problems/no/1962
// Tags: fps, inclusion-exclusion-principle, invplus, inverse-of-repetition, nodup
fn main() {
    let n: usize = get();
    let m: i64 = get();
    let mut g = vec![MInt::new(0); n + 1];
    g[0] += 1;
    let coef = [MInt::new(0), MInt::new(1), 1.into(), 0.into(), (MOD - 1).into(), (MOD - 1).into()];
    for i in 1..n + 1 {
        let b = std::cmp::max(m - i as i64 + 1, 0);
        g[i] = -coef[i % 6] * b;
    }
