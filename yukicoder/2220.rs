...
fn main() {
    ...
    let mut ev = vec![vec![]; q + 1];
    for (l, r, a) in lra {
        if a >= n { continue; }
        let l = x.lower_bound(&l);
        let r = x.upper_bound(&r);
        ev[l].push((a, 1));
        ev[r].push((a, -1));
    }
    for i in 0..q {
        for &(pos, kind) in &ev[i] {
            let old = st.query(pos..pos + 1);
            st.update(pos, old + kind);
        }
        let idx = st.max_right(0.., &|x| x != 0);
        puts!("{}\n", idx);
    }
}
