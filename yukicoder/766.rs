...
fn solve() {
    ...
    let p = ModInt::new(p) * ModInt::new(100).inv();
    let mut v = v;
    v.sort();
    v.reverse();
    const W: usize = 1 << 19;
    ...
    for i in 0 .. n {
        acc[i + 1] = acc[i] + ModInt::new(v[i]);
    }
    let mut rest = ModInt::new(1);
    for i in 0 .. n {
        let tmp = fac[i + m - 1] * invfac[m - 1]
            * invfac[i] * (ModInt::new(1) - p).pow(i as i64)
            * p.pow(m as i64 - if i == n { 1 } else { 0 });
        tot += tmp * acc[i];
        rest -= tmp;
    }
    tot += rest * acc[n];
