...
// https://yukicoder.me/problems/no/2413 (4)
// A = 1 + x + ... + x^9, B = 1 + x^10 + ... + x^90 とする。
// 問題の答えは \sum_{0 <= i,j <= 9N, 99|i+10j}(i+j)^K[x^i]A^{ceil(N/2)}[x^j]A^{floor(N/2)} である。
// -> 解説を見た。A^? を計算してから、次数 mod 11 で見て畳み込みをする。
// 計算量は O(11 K log K) where K = 9N である。
...
fn main() {
    ...
    let mut g1 = fps_ln(&f, 3.into(), &fac, &invfac);
    let mut g2 = g1.clone();
    for i in 0..W {
        g1[i] *= (n as i64 + 1) / 2;
        g2[i] *= n as i64 / 2;
    }
    let h1 = fps_exp(&g1, 3.into(), &fac, &invfac);
    let h2 = fps_exp(&g2, 3.into(), &fac, &invfac);
    let factor = MInt::new(W as i64).inv();
    let zeta = MInt::new(3).pow((MOD - 1) / W as i64);
    let mut ans = MInt::new(0);
    let mut c1 = vec![MInt::new(0); W];
    let mut c2 = vec![MInt::new(0); W];
    for rem in 0..11 {
        for i in 0..W {
            if i % 11 == rem {
                c1[i] = h1[i];
        ...
        fft::fft(&mut c1, zeta, 1.into());
        fft::fft(&mut c2, zeta, 1.into());
        for i in 0..W {
            c1[i] *= c2[i] * factor;
        }
        fft::inv_fft(&mut c1, zeta.inv(), 1.into());
        for i in 0..W {
            if i % 9 == 0 {
                ans += MInt::new(i as i64).pow(k as i64) * c1[i];
            }
        }
    }
    println!("{}", ans);
