...
fn conv(a: &[MInt], b: &[MInt]) -> Vec<MInt> {
    let n = a.len() - 1;
    let m = b.len() - 1;
    let mut p = 1;
    while p <= n + m { p *= 2; }
...
fn main() {
    ...
    let mut ccoef = vec![vec![MInt::new(0); n]; 5];
    for i in 0..n {
        ccoef[c[i]][i] += 1;
    }
    let mut coef = vec![vec![MInt::new(0); n]; 5];
    for &(a, b, y) in &aby {
        coef[b][n - 1 - a] += y;
    }
    let mut sum = vec![0; n + 1];
    for i in 0..5 {
        let t = conv(&ccoef[i], &coef[i]);
        for j in 0..n {
            sum[j] += t[n - 1 + j].x;
        }
    }
    let mut ans = sum[0];
    for i in 0..n + 1 {
        ans = std::cmp::max(ans, sum[i] + i as i64 * x);
    }
    println!("{}", ans);
}
